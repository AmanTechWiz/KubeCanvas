...
- feature [37] — Canvas storage migration (complete)
...
- Chat agent (`/api/ai/chat`) reads canvas state via Liveblocks Node SDK `getStorageDocument()` (not raw fetch) — same pattern as design-agent for consistency; canvas summary injected into system prompt at runtime
...
- Chat and architect agents have separate system prompt files: `trigger/chat_system_prompt.ts` (advisor, discusses) and `trigger/ai_system_prompt.ts` (builder, generates patch operations)
- Chat agent is topic-locked to system design/infrastructure/databases/cloud — refuses off-topic requests; architect agent is task-locked to generating architecture diagrams
