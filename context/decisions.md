...
- 7 shapes: rectangle, diamond, circle, pill, cylinder, hexagon, text
- Text shape nodes are standalone — no border, no background, no connection handles, no resize handles
- Text shape supports double-click-on-canvas to create at clicked position with auto-edit
- `NodeResizeContext` provides `resizeNode()` to write dimensions to Liveblocks Storage from custom nodes
...
- Edge handle direction computed from **node centers** (x+w/2, y+h/2), not top-left corners — ensures accurate bottom→top / right→left routing regardless of layout direction
- Handle IDs use `id={position}` (e.g. `"top"`, `"bottom"`) — React Flow scopes source/target separately, no collision
...
- `manualSave()` clears `lastSavedRef.current` before calling `save()` to force `attemptSave` past the "nothing changed" guard — ensures clicking Save always triggers a real save even if autosave just saved the same state
...
## Canvas Snapshot History (Photo Album Undo)

- **Replaces Liveblocks client-local undo/redo** with app-level snapshot history because Liveblocks' native stack only records local browser ops — server-side AI mutations are invisible and stale inverse ops corrupt the canvas across AI edits.
- Every settled change (human gesture, template import, Clear All, collaborator edit, AI run) becomes one undoable photo of the entire canvas.
- AI run boundary detected via `agentThinking` Storage flag (`false → true` = push pre-snapshot, completion = push post-snapshot).
- `history.disable(() => restoreMutation(snapshot))` prevents the restore from polluting Liveblocks' native undo stack.
- 50 snapshots max (FIFO eviction).
- System mutations (CanvasLoader, cleanup, edge migration) explicitly excluded from history.

---

## Canvas Cleanup Layout (dagre)

- **Direction detection**: edge handle orientation is the primary signal (3x weight) because handles encode explicit directional intent (bottom→top = TB, right→left = LR). Position deltas are secondary (1x weight). This prevents the direction from flipping randomly between cleanup runs.
- **Spacing scales with graph size** across 4 tiers (1–6, 7–12, 13–20, 20+ nodes) — small graphs stay tight, large architectures don't cramp.
- **Handle computation is direction-aware** — dagre's rankdir determines which side source/target handles go on (TB → bottom/top, LR → right/left, etc.).
- **Auto-offset**: after dagre, positions shifted so top-left node starts at (60,60) — no nodes bleed into negative coordinates.
- **Auto-fit**: `fitView({ padding: 0.2 })` fires after cleanup to immediately show the cleaned layout.
- Implemented in standalone `lib/canvas-cleanup.ts` — not coupled to the AI layout engine (`lib/layout-architecture.ts`).

---

## Natural LLM-Generated Summaries

- AI modification summaries are now generated by a fast Gemini call (`generateText`) instead of a template that concatenates diff counts.
- System prompt: "ONE sentence only, max 25 words. Be conversational and natural."
- Examples: "I've added a Redis cache between the API Gateway and your services." / "Designed a microservices architecture with API Gateway, Auth, and PostgreSQL."
- Fallback: count-based summary when LLM call fails (e.g. "Updated 5 components, added Redis layer").
- Implemented as `generateNaturalSummary()` in `trigger/design-agent.ts`.

---

## Deployment Prep

- **CI/CD**: GitHub Actions for lint → typecheck → build → deploy.
- **Trigger.dev prod deploy**: separate `trigger.config.ts` profile with production environment variables.
- **Prisma migration strategy**: run `prisma migrate deploy` as part of CI/CD (not `prisma migrate dev` which is dev-only).
- **Environment variables**: audit SECRET_KEY vs public NEXT_PUBLIC_* split for Vercel/cloud provider.
- **Canvas persistence**: Prisma `canvasJson` field is the durable source — Liveblocks rooms are ephemeral with TTL.
...
- Manual save no-op: `lastSavedRef.current` cleared in `manualSave()` to force past the "nothing changed" guard
- Spec export stuck at "Generating": `phaseLabel` ternary reordered to check `isDone` before phase — previously `phase === "complete"` was checked before `isDone`, causing the label to stay stuck when realtime didn't deliver the final phase
...
## Position Preservation During AI Modifications

- When the AI modifies an existing architecture, **existing node positions are always preserved** — no dagre reset
- **New nodes** are positioned via `computeNewNodePositions()`, not dagre:
  - Phase 1: nodes with existing connections → placed at centroid of connected existing nodes + 200px offset, staggered for siblings
  - Phase 2: BFS from placed nodes for new-to-new chains (each 200px below its parent)
  - Phase 3: orphan unconnected nodes → placed to the right of the rightmost existing node
- Edge handles are **recomputed** from the final positions so routing stays correct
- The `diff.updateNodes` loop in `design-agent.ts` no longer calls `flow.updateNode(id, { position })` — only data changes (label, color, shape, logo) are applied
- Fresh generation (empty canvas) flow is unchanged — dagre TB on all nodes
- `layoutArchitecture()` accepts optional `existingPositions?: Map<string, {x, y}>` parameter; fresh callers omit it for default behavior

## Layout Direction Support

- Added optional `direction` field to `ArchitectureSchema` (`"TB" | "LR" | "BT" | "RL"`)
- **Fresh generation**: AI sets `direction` when user requests a layout direction; dagre uses it as `rankdir`
- **Modifications**: `computeNewNodePositions()` infers direction per-anchor from existing node positions:
  - Pre-computes edge direction (horizontal vs vertical) for every existing-to-existing edge
  - Each existing node votes "x" or "y" based on majority of its connected edges
  - New nodes connected to "x"-dominant anchors → placed to the RIGHT, stagger DOWN
  - New nodes connected to "y"-dominant anchors → placed BELOW, stagger RIGHT
- `ranksep` adjusted for horizontal flows (240px vs 120px for TB) to prevent node overlap
- AI prompts (FRESH_PROMPT, MODIFY_PROMPT, ai_system_prompt.ts) instruct the LLM to set direction based on natural language input
